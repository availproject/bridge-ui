...
export function parseError(error: any): string {
  ...
  if (errorMessageString.match(/Network Error/i)) {
  ...
  if (
    errorMessageString.match(
      /Cannot read properties of null (reading 'signature')/i,
    )
  ) {
  ...
  if (
    errorMessageString.match(
      /does not match the target chain for the transaction/i,
    )
  ) {
