  ...
  const initWormholeBridge = useCallback(
    async ({
      ...
      try {
        ...
        if (!srcNttContracts) {
          throw new Error(
            `NTT contracts not found for chain ${sendChain.chain}`,
          );
        }
        if (!dstNttContracts) {
          throw new Error(
            `NTT contracts not found for chain ${rcvChain.chain}`,
          );
        }
        ...
        const srcNttExecutor = await sendChain.getProtocol("NttWithExecutor", {
          ntt: srcNttContracts,
        });
        ...
        if (walletClient) {
          ...
          const srcAddress = Wormhole.chainAddress(
            sendChain.chain,
            signer.address(),
          );
          ...
          if (!validated.valid) {
            throw new Error(
              `Validation failed: ${validated.error?.message || "Unknown error"}`,
            );
          }
          ...
          const routeQuote = await routeInstance.fetchExecutorQuote(
            tr,
            validatedParams,
          );
